...
class MyPromise {
	...
	static race(promises){
		return new MyPromise((resolve,reject)=>{			
			for(let i=0;i < promises.length;i++){
				let p = promises[i];
				if(p.then && typeof p.then === 'function'){
					p.then(resolve,reject) // 一旦成功就直接改变状态了
				}else{
					resolve(p)
				}
			}
	...
	static all(promises){
		return new MyPromise((resolve,reject)=>{
			...
			const processSuccess = (index,value)=>{
				results[index] = value;
				/* 
					这里不能以results.length === promises.length去判断
					因为promises假如是[promise,promise,3]
					此时3是最先执行完成的，所以就会出现：
					results[2] = 3;
					[undefined,undefiend,3]
					如果基于length判断：results.length = 3 以为符合条件了 其实前两个异步结果还没有返回
					所以这里得借助于计数器
				 */
				if(counter === promises.length){
					resolve(results);
	...
	then(onFulfilled, onRejected) {
		...
		return this.then((data)=>{
			/* 
				使用MyPromise.resolve的原因在于只要这个Api可以等到传入的promsie执行完成，有一个延迟的效果
				使用then的原因是我们并不想要MyPromise.resolve返回的这个promsie中的成功还是失败的结果 这是finally的api特性决定的 我们只要上一个调用finally的promise的结果，所以这里必须包装一层 
				之所以可以包装就是then方法返回的还是promsie  外面可以接着then
			 */
			return MyPromise.resolve(onFinally()).then((n)=>{
				console.log("n1: ",n);
				return data;
			},(m)=>{
				console.log("m1: ",m);
